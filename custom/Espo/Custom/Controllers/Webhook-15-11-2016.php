...
class Webhook extends \Espo\Core\Templates\Controllers\Base
{
	public function actionGenerate($params, $data, $request, Entity $entity) {
		...
		foreach ($parameters as $strKey => $strInputData) {	
			...
			if($strKey == 'lastname' && !empty( $strInputData )) {
				$paramflag = true;
			} 
			...
			if($strKey == 'phonenumber' && !empty( $strInputData )) {
				$paramflag = true;
			} 		
	    ...
	    if( false == $paramflag) {
		   	return array( 'result' => 'false' , 'msg' => 'Missing input parameters.');
	    }   
			
		
	 	$queryName = " select count(l.id) as record 
			from lead l
			join entity_email_address ema ON ( ema.entity_id = l.id )
			...
			OR ( ea.name = '".$parameters['emailaddress']."')	OR ( pn.name = '".$parameters['phonenumber']."')
			and ema.entity_type = 'Lead' and ema.primary = '1'
			and epn.entity_type = 'Lead' and epn.primary = '1'
			and l.deleted='0' and pn.deleted='0' and ea.deleted=0
			";
